...
# does, so "vicars scan to email" matches the stored "Vicars-Scan-To-Email".
set -u
...
case "$cmd" in
  archive|revive)
    ...
    # actual stored names, so any casing/spacing matches (e.g. "ninja mcp play" -> "Ninja-MCP-Play").
